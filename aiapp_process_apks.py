...
# @Time    : 2023/4/2 9:56
# @Author  : Jin Au-yeung
# @File    : aiapp_process_apks.py
# @Software: PyCharm
# aiapp_process_apks.py
import multiprocessing
import os
import glob
from concurrent.futures import ProcessPoolExecutor, as_completed
from aiapp_process_apk_module import process_apk
import config

# 指定存放 APK 文件的目录
apk_directory = config.ROOT_PATH + '/apks/'
...
if __name__ == '__main__':
    ...
    with open('checked_apps.txt', 'r') as f:
        # 读取已经分析的文件中所有的.apk文件名
        apk_filenames = []
        for line in f:
            apk_path = line.split(',')[0]
            apk_filenames.append(apk_path)

    # 假设有一个列表apps
    # 删除已读取的.apk文件名
    for apk_filename in apk_filenames:
        if apk_filename in apk_files:
            apk_files.remove(apk_filename)
    print(len(apk_files))
